...
the frontend** via `scheduler.invoke_on_owner` (synchronous hop: same call
shape, result/exception returned to the worker). In single-threaded / inline
...
- Optional **`retry_callback`**: raiser attaches a one-shot closure that resumes
  ...
  ``retry()`` runs and clears it. Accept→streams runs ``pending.retry()`` on the
  owner via ``invoke_on_owner`` rather than re-running full ``open_pair``
  (avoids dropping eager-drained bytes).

IOManager uses `_call_with_frontend_retry` / `invoke_on_owner` on paths that
**continue after a completion** and may run on workers: accept preread (advance
and `_sock_accept_preread`), `_attach_sock_sendall` (connect advance),
`_sock_sendall_from_offset` (short re-arm from send complete), accept_many
worker open, `_accept_preread_on_worker`. On hop: use `exc.retry` when cargo is
set, else re-run the call. Fire-and-forget posts still use
...
|-------|-------|-----------|-----------------|
| Accept-time preread | `accept_many` / accept | `proactor.recv` after eager miss | **worker delivery:** `_call_with_frontend_retry(recv)` then wire timeouts |
| Oneshot accept + recv | `sock_accept` + group | `proactor.recv` on **accept advance** | `_call_with_frontend_retry(recv)` then attach |
| Eager accept + preread | `sock_accept` ready path | `_sock_accept_preread` | `_call_with_frontend_retry(recv)` (safe if ever on worker; no-op hop on owner) |
| Connect + initial | connect **advance** | `_attach_sock_sendall` send arms | `_call_with_frontend_retry(send)`; short re-arm same path |
| `sock_sendall` | first arm owner; re-arm on send complete | `_sock_sendall_from_offset` | every proactor ``send`` via `_call_with_frontend_retry` (re-arm may be worker) |
| Accept + streams | `accept_many` | **Worker delivery:** `_call_with_frontend_retry(open_pair)`. **Eager ready drain:** bare `open_pair`. Custom `stream_factory` must re-wrap so `retry()` returns a pair (see `StreamFactory`) |
...
3. On **`RetryOnFrontend`** (continuation paths only) →
   `_call_with_frontend_retry` hops `exc.retry` or the submit to the owner;
   caller continues with the result or exception. Bare initial arms and sendall
   do not hop.
