...
Shared by ``ProactorIOManager`` (accept/create-stream paths) and the public
``streams`` package. Lives outside ``streams/`` so ``io_manager`` does not
import the streams API surface.
"""
...
from .stream_reader import AsyncStreamReader, StreamReader
from .stream_writer import AsyncStreamWriter, StreamWriter, StreamWriterIO
...
class StreamOpenIO(Protocol):
    """``ProactorIOManager`` slice needed to open stream buffers on a socket."""
