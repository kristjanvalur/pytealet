...
class _QueueingScheduler(StubScheduler):
    """Record ``call_soon_threadsafe`` instead of running it (async post tests).

    ``invoke_on_owner`` still runs immediately (sync hop stand-in for unit tests).
    """

    def __init__(self) -> None:
        super().__init__()
        self.queued: list[tuple[Any, tuple[object, ...]]] = []
        self.invoke_calls: list[tuple[Any, tuple[object, ...]]] = []
    ...
    def invoke_on_owner(self, callback, *args: object) -> object:
        self.invoke_calls.append((callback, args))
...
class TestProactorIOManagerAcceptMany:
    ...
    def test_accept_many_preread_retries_recv_via_invoke_on_owner(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Recovery: first proactor.recv raises RetryOnFrontend; redo on owner."""
        ...
        try:
            ...
            # Sync hop re-arms recv; delivery is still posted via call_soon.
            assert recv_attempts["n"] == 2
            assert len(scheduler.invoke_calls) >= 1
            scheduler.drain()
    ...
    def test_accept_many_streams_retries_open_via_invoke_on_owner(self) -> None:
        """Recovery: backend ``RetryOnFrontend`` on open retries via ``invoke_on_owner``."""
        ...
        try:
            ...
            assert len(scheduler.invoke_calls) >= 1
            # stream callback is still posted via call_soon (reorder path)
            scheduler.drain()
            assert handled
            _reader, writer = handled[0]
            writer.close()
    ...
    def test_accept_many_streams_retry_preserves_eager_recv_bytes(self) -> None:
        ...
        try:
            ...
            assert recv_many_attempts["n"] == 2
            assert len(scheduler.invoke_calls) >= 1
            scheduler.drain()  # stream callback delivery
            assert handled
...
class TestProactorIOManagerAcceptEager:
    ...
    def test_accept_many_drains_ready_then_submits_continuous(self) -> None:
        class _CaptureProactor(_MockProactor):
            def __init__(self) -> None:
                super().__init__()
                self.accept_many_calls = 0
                self.last_base_sequence = -1
                self.last_accept_callback: Any = None

            def accept_many(self, sock: socket.socket, callback=None, *, base_sequence: int = 0):
                self.accept_many_calls += 1
                self.last_base_sequence = base_sequence
                self.last_accept_callback = callback
        ...
        try:
            ...
            assert proactor.last_base_sequence == 3
    ...
    def test_accept_many_empty_queue_only_submits(self) -> None:
        class _CaptureProactor(_MockProactor):
            def __init__(self) -> None:
                super().__init__()
                self.accept_many_calls = 0
    ...
    def test_accept_many_eager_preread_drains_then_submits(self) -> None:
        ...
        try:
            ...
            assert all(data == b"hi" for _conn, data in accepted)
            for conn, _data in accepted:
                conn.close()
        finally:
            for client in clients:
                client.close()
    ...
    def test_accept_many_streams_drains_ready(self) -> None:
        from tealetio.streams import StreamReader, StreamWriter
    ...
    def test_sock_accept_returns_sync_when_ready(self) -> None:
        proactor = _MockProactor()
        io = _manager(proactor)
...
class TestProactorIOManagerDirect:
    ...
    def test_sock_connect_initial_send_retries_via_invoke_on_owner(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Connect advance → ``_attach_sock_sendall`` hops RetryOnFrontend send."""
    ...
    def test_sock_accept_closes_conn_when_preread_attach_returns_none(
        ...
        def attach_reject_recv(
            ...
            if operation.kind == "recv":
                ...
                IOWaiter(io, io._proactor.cancel(operation)).forget()
    ...
    def test_sock_accept_closes_conn_when_invoke_on_owner_attach_rejects(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Quiet attach during owner-hop recovery must not leak the accepted socket."""
        ...
        def attach_reject_after_retry(
            self: IOWaitGroup[Any], operation: Operation[Any], **kwargs: Any
        ) -> Any:
            # First attach is accept (succeeds); preread recv is only after hop.
            if operation.kind == "recv":
                on_cleanup = kwargs.get("on_cleanup")
                IOWaiter(io, io._proactor.cancel(operation)).forget()
                if on_cleanup is not None:
        ...
        monkeypatch.setattr(IOWaitGroup, "attach", attach_reject_after_retry)
        try:
            waiter = io.sock_accept(listen, 64)
            assert isinstance(waiter, IOWaitGroup)
            assert len(accepted) == 1
            assert recv_attempts["n"] == 2  # raise + hop retry
            assert len(scheduler.invoke_calls) >= 1
            assert accepted[0].fileno() == -1
        finally:
            for peer in peers:
    ...
    def test_sock_create_closes_socket_when_send_retry_attach_rejects(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """Quiet attach after hopped send recovery must not leak the socket."""
