...
class TestStreamsPoC:
    ...
    def test_start_server_closes_listener_socket_when_bind_fails(self, scheduler: SyncProactorScheduler) -> None:
        ...
        def exercise() -> None:
            blocker = scheduler.io.sock_create(socket.AF_INET, socket.SOCK_STREAM).wait()
    ...
    def test_open_connection_resolves_literal_ip_inside_scheduler(self, scheduler: SyncProactorScheduler) -> None:
        ...
        try:
            ...
            def accept_and_echo() -> None:
                ...
                try:
                    payload = scheduler.io.sock_recv(conn, len(client_greeting)).wait()
                    scheduler.io.sock_sendall(conn, payload.upper()).wait()
                finally:
                    conn.close()
    ...
    def test_open_connection_resolves_hostname_via_scheduler(self, scheduler: SyncProactorScheduler) -> None:
        ...
        try:
            ...
            def accept_and_echo() -> None:
                conn, _initial = scheduler.io.sock_accept(server).wait()
                try:
                    scheduler.io.sock_sendall(conn, b"PONG").wait()
                finally:
                    conn.close()
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                def accept_and_echo() -> None:
                    conn, _initial = scheduler.io.sock_accept(server).wait()
                    try:
                        payload = scheduler.io.sock_recv(conn, 5).wait()
                        scheduler.io.sock_sendall(conn, payload.upper()).wait()
                    finally:
                        conn.close()
