...
2. If `get_sqe_try` succeeds: prep the next `IORING_OP_SEND` (same Completion
   ...
   cannot stall; otherwise leave the SQE for the next harvest flush or host
   `submit()`. Never enter from a non-issuer.
...
- Send-all CQE path: if the SQ has a slot, **fill** the next-leg even on a
  worker. Park only when there is no slot. Submit that SQE only if this
  thread may enter and a unique waiter is already held (deadlock if the
  waiter is in `wait_cqe`). Otherwise harvest flush or host `submit()`.
