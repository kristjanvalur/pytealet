...
def test_ring_prepare_read_rejects_negative_offset():
    require_uring()

    with uring_api.Ring() as ring:
        with pytest.raises(ValueError, match="offset must be non-negative"):
            ring.prepare_read(0, bytearray(1), -1)

def test_ring_prepare_write_rejects_negative_offset():
    require_uring()

    with uring_api.Ring() as ring:
        with pytest.raises(ValueError, match="offset must be non-negative"):
            ring.prepare_write(0, b"x", -1)

def test_ring_prepare_read_rejects_buffer_length_above_uint_max():
    require_uring()
    ...
    try:
        with uring_api.Ring() as ring:
            with pytest.raises(ValueError, match="buffer length must fit in uint32_t"):
                ring.prepare_read(0, buf, 0)
    finally:
        buf.close()

def test_ring_prepare_write_rejects_buffer_length_above_uint_max():
    require_uring()
...
def test_ring_file_read_write_completion_when_available():
    ...
    try:
        ...
        try:
            with uring_api.Ring() as ring:
                write_handle = ring.prepare_write(fd, b"hello", 0, token)
                ...
                buf = bytearray(5)
                read_handle = ring.prepare_read(fd, buf, 0, token)
...
def test_ring_prepare_statx_rejects_small_buffer():
    require_uring()

    with uring_api.Ring() as ring:
        with pytest.raises(ValueError, match="256 bytes"):
            ring.prepare_statx(0, "", 0, uring_api.STATX_SIZE, bytearray(32))
...
def test_ring_statx_path_returns_size_in_buffer_when_available():
    ...
    try:
        ...
        with uring_api.Ring() as ring:
            statx_handle = ring.prepare_statx(uring_api.AT_FDCWD, path, 0, uring_api.STATX_SIZE, buf, token)
...
def test_ring_statx_without_size_mask_returns_none_result_when_available():
    ...
    try:
        ...
        buf = bytearray(uring_api.STATX_BUFFER_SIZE)
        with uring_api.Ring() as ring:
            handle = ring.prepare_statx(uring_api.AT_FDCWD, path, 0, statx_nlink, buf)
            completion = wait_one(ring, 1.0)
            assert completion is handle
            assert completion.kind == uring_api.COMPLETION_KIND_STATX
            assert completion.res == 0
...
def test_ring_statx_fdsize_returns_size_in_completion_result_when_available():
    ...
    try:
        ...
        try:
            assert os.write(fd, b"hello") == 5
            with uring_api.Ring() as ring:
                handle = ring.prepare_statx_fdsize(fd, token)
                completion = wait_one(ring, 1.0)
                assert completion is handle
...
def test_ring_statx_fdsize_fails_for_invalid_fd():
    ...
    with uring_api.Ring() as ring:
        handle = ring.prepare_statx_fdsize(-1)
        completion = wait_one(ring, 1.0)
        assert completion is handle
        assert completion.kind == uring_api.COMPLETION_KIND_STATX_FDSIZE
        assert completion.res < 0
...
def test_ring_openat_read_write_round_trip_when_available():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        with uring_api.Ring() as ring:
            ...
            close_handle = ring.prepare_close(fd, token)
