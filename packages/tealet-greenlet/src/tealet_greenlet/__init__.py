"""Greenlet-compatible API built on top of tealet."""
    ...
    def __exit__(self, _, val, tb):
        ...
        try:
            ...
            if isinstance(val, _tealet.TealetError):
                ...
                if "set_pending_exception() not allowed from a different thread" in msg or msg.startswith(
                    "thread mismatch:"
                ):
...
def install(force=True):
    """Expose tealet_greenlet as importable top-level greenlet modules."""
